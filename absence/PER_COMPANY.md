...
| **Goldman Sachs** | 2 / 7 | **C** | Both verified cells correct: assurance correctly `absent` (PwC assures bond proceeds, not emissions — see correction above) and `scenario_analysis_quantified` correctly absent in run 3, though run 4 produced a confident false positive on that same cell, so the item is volatile here. |
