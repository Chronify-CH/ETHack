...
Three files matter. This is a guide to all three, plus the traps to avoid
when reading them.
...
- Apple's `target_net_zero_year` came back `absent` at 0.069 in run 3, with a
  ...
  score was simply wrong. (Fixed in the current large-model run -- but it's
  the clearest illustration of why the label alone isn't enough.)
...
2. Read the `snippet`. Does it actually say what the item's `hypothesis` (in
   `items.py`) claims?
3. If you're unsure, `grep` the company's raw text file in
   `absence/data/raw/<slug>.txt` for the relevant keywords yourself -- that's
   exactly the manual verification method `MILESTONE1_RESULTS.md`'s audits
   used, and it's available to you the same way it was available here.

## Comparing across the three runs
...
The lesson generalizes: when auditing any future run, check retrieval
(did the right paragraph get considered at all?) before blaming the model.
